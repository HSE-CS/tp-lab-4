...
void Automata::choice(string product) {
	if (this->state == ACCEPT || this->state == CHECK) {
		...
		if (it == this->menu.end()) {
			...
			(*input_stream) << "You have chosen " << product << endL
		...
		if (flag) {
			if (this->money >= this->menu[product]) {
				...
				(*input_stream) << "Not enough money" << endL;
			}
		}
	}
}
...
void Automata::cook() {
	if (this->state == CHECK) {
		...
		(*input_stream) << "You bought : " << (*it).first << endL;
		finish();
	}
}
...
void Automata::printMenu() {
	...
	if (this->state != OFF) {
		(*input_stream) << "__________Menu__________" << endL;
		map <string, int> ::iterator it;
